...
namespace Nebukam.JobAssist
{
    public static class Extensions
    {
        ...
        {
            if (@this.TryGetFirstValue(key, out TValue result, out NativeParallelMultiHashMapIterator<TKey> it))
            {
            ...
            where TValue : unmanaged
        {
            ...
            {
                TKey key = keys[k];
                if (@this.TryGetFirstValue(key, out TValue value, out NativeParallelMultiHashMapIterator<TKey> it))
                {
        ...
        public static bool Copy<T>(T[] src, ref NativeArray<T> dest, Allocator alloc = Allocator.Persistent)
            where T : unmanaged
        {
            bool resized = MakeLength(ref dest, src.Length, alloc);
    ...
    [BurstCompile]
    internal struct FloodArray<T> : IJobParallelFor
        where T : unmanaged
    {
